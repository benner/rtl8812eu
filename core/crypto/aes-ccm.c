			       ...
			       u8 *x)
{
	...
	rtw_crypto_aes_encrypt(aes, b, x); /* X_1 = E(K, B_0) */
	...
	rtw_crypto_aes_encrypt(aes, aad_buf, x); /* X_2 = E(K, X_1 XOR B_1) */

	if (aad_len > AES_BLOCK_SIZE - 2) {
		xor_aes_block(&aad_buf[AES_BLOCK_SIZE], x);
		/* X_3 = E(K, X_2 XOR B_2) */
		rtw_crypto_aes_encrypt(aes, &aad_buf[AES_BLOCK_SIZE], x);
	}
}
...
static void aes_ccm_auth(void *aes, const u8 *data, size_t len, u8 *x)
{
	...
	for (i = 0; i < len / AES_BLOCK_SIZE; i++) {
		...
		data += AES_BLOCK_SIZE;
		rtw_crypto_aes_encrypt(aes, x, x);
	}
	if (last) {
			 ...
			 u8 *a)
{
	...
	for (i = 1; i <= len / AES_BLOCK_SIZE; i++) {
		...
		/* S_i = E(K, A_i) */
		rtw_crypto_aes_encrypt(aes, a, out);
...
static void aes_ccm_encr_auth(void *aes, size_t M, u8 *x, u8 *a, u8 *auth)
{
	...
	rtw_crypto_aes_encrypt(aes, a, tmp);
	for (i = 0; i < M; i++)
		auth[i] = x[i] ^ tmp[i];
	       ...
	       const u8 *aad, size_t aad_len, u8 *crypt, u8 *auth)
{
	...
	aes = rtw_crypto_aes_encrypt_init(key, key_len);
	if (aes == NULL)
		return -1;
	...
	rtw_crypto_aes_encrypt_deinit(aes);
