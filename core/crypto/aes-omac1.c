		     ...
		     const u8 *addr[], const size_t *len, u8 *mac)
{
	...
	ctx = rtw_crypto_aes_encrypt_init(key, key_len);
	if (ctx == NULL)
		return -1;
	...
	while (left >= AES_BLOCK_SIZE) {
		...
		if (left > AES_BLOCK_SIZE)
			rtw_crypto_aes_encrypt(ctx, cbc, cbc);
		left -= AES_BLOCK_SIZE;
	}

	os_memset(pad, 0, AES_BLOCK_SIZE);
	rtw_crypto_aes_encrypt(ctx, pad, pad);
	gf_mulx(pad);
	...
	for (i = 0; i < AES_BLOCK_SIZE; i++)
		pad[i] ^= cbc[i];
	rtw_crypto_aes_encrypt(ctx, pad, mac);
	rtw_crypto_aes_encrypt_deinit(ctx);
	return 0;
}
